    ...
    pub fn freeze(self) {
        ...
        let hclk = match self.mux {
            ...
            ClockSrc::PLL(div) => {
                ...
                unsafe {
                    let p_reg = 0x40001008 as *mut u8;
                    with_safe_access(||{
                        // ptr::write_volatile(0x40001008 as *mut u8, 0x48);
                        *p_reg = 0x48;
                ...
                // with_safe_access(|| unsafe {
                //     sys.clk_sys_cfg()
                //         .write(|w| w.clk_pll_div().bits(0x48));
                //     riscv::asm::nop();
                //     riscv::asm::nop();
                //     riscv::asm::nop();
                //     riscv::asm::nop();
                ...
                // with_safe_access(|| unsafe {
                //     sys.clk_sys_cfg()
                //         .write(|w| w.clk_sys_mod().bits(0b01).clk_pll_div().bits(0x48));
                //     riscv::asm::nop();
                //     riscv::asm::nop();
                //     riscv::asm::nop();
                //     riscv::asm::nop();
        ...
        let dupa = sys.pll_config().read().bits() | (1 << 7);

        with_safe_access(|| unsafe {
            sys.pll_config().write(|w| w.bits(dupa));
        });
